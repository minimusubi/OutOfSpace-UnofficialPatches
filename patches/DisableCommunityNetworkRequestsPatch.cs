...
namespace UnofficialPatches.patches {
     ...
     * There are three network requests made for update notes, and one to check if BeholdStudios is live on Twitch.
     * All four of these network requests fail. The update notes endpoints do not work, and the Twitch API endpoint is outdated.
     * 
     * These network requests may be harmless, but since they are no longer maintained, it seems safer to simply remove them.
     ...
     * Since I'm not sure that Behold Studios will always remain in control of these, I believe it's safer to simply stop
     * these requests.
     * 
     * The Twitch API used was the now deprecated Kraken API.
